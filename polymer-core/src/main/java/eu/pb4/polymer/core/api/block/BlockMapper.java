...
public interface BlockMapper {
    Event<BiFunction<PacketContext, BlockMapper, @Nullable BlockMapper>> DEFAULT_MAPPER_EVENT = EventFactory.createArrayBacked(BiFunction.class, arr -> (ctx, mapper) -> {
        for (var c : arr) {
            var newMapper = c.apply(ctx, mapper);
    ...
    static BlockMapper getDefault(PacketContext context) {
        return DEFAULT_MAPPER_EVENT.invoker().apply(context, BlockMapperImpl.DEFAULT);
    }
